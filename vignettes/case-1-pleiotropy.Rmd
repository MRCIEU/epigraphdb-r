...
- For a list of genes that we wish to assess their pleiotropic profiles (against a variant of interests), we retrive from EpiGraphDB their mapped protein products.
...
pathway <- shared_pathway_df %>% pull(shared_pathway) %>%
    unlist() %>% unique()
...
For each pair of proteins, we retrieve shared protein-protein interactions, either a direct interaction or an interaction where there's one mediator protein in the middle (see paramaters section).
