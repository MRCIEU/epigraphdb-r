---
title: "Using EpiGraphDB Web API"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{"Using EpiGraphDB Web API"}
...
[RESTful API service](http://api.epigraphdb.org).

EpiGraphDB [web application](http://epigraphdb.org):

![web application](figures/epigraphdb-xqtl-view.png)

EpiGraphDB [web API (OpenAPI / swagger interface)](http://api.epigraphdb.org):
...
r <- httr::GET(glue::glue("{url}/mr"), query = payload)
r %>% httr::content(as = "parsed") %>% str(2)
```
...
```{r epigraphdb-mr}
epigraphdb::mr(exposure_trait = "Body mass index",
               outcome_trait = "Coronary heart disease",
               mode = "raw") %>%
  str(2)
```
...
```shell
curl -X 'GET' 'http://api.epigraphdb.org/mr?exposure=Body+mass+index&outcome=Coronary+heart+disease' | jq
```
...
- [command line: httpie](https://github.com/jakubroztocil/httpie)
- [python: requests](https://realpython.com/python-requests/)
- [GUI client: insomnia](https://support.brightcove.com/use-insomnia-api-requests)
...
please visit our [documentation site](http://docs.epigraphdb.org).
