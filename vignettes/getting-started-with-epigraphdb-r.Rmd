...
* `endpoint` is the 'address' within EpiGraphDB of where you want to query the data from, and there is a specific one for each case. e.g.  `"/mappings/gene-to-protein"` is used to query gene-to-proteins mappings. See the full list of currently available endpoints [here][2]

* `params` is where you supply the list of things you want to query about (can be a list of genes, GWAS trait, MR outcome of interest)
...
proteins_uniprot_ids <- c("P04637") %>% I() # use I()/(AsIs) function when providing a list of one
...
The list of nodes is also available in the [documentation][11], along with the node properties, which are useful for running native Cypher queries. In this section, we want to show how to perform node search for a term of interest. Any node can be searched by specifying it in this query: `/meta/nodes/{meta_node}/search`.  
...
The functionality of `epigraphdb` R package is currently limited to a certain number of endpoints available via the `query_epigraphdb` function. If you would like to access one of the endpoints that are currently [not directly available][2] through it, we recommend exploring Neo4j Cypher query language to construct native EpiGraphDB queries. 
...
It is important to note, that not all parameters can be queried using Cypher due to the underlying data types. For example, the query above could not be modified to filter by `node.sample_size` because in the underlying dataset (_currently!_) the sample_size column stores data as strings. Please refer to [data dictionary][7] to explore data types before writing native Cypher queries. In the meantime, such filtering will have to be done post-query in R. 
