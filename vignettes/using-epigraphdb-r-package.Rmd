...
[upstream API endpoints](http://api.epigraphdb.org)
for users to use.
...
```{r}
df <- mr(exposure_trait = "Body mass index",
...
- The API Swagger interface http://api.epigraphdb.org
- The documentation on API endpoints http://docs.epigraphdb.org/api/api-endpoints/
...
```{r}
response <- mr(exposure_trait = "Body mass index",
               outcome_trait = "Coronary heart disease",
               mode = "raw")
response %>% str(2)
```
